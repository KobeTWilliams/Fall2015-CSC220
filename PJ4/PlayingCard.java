...
class Deck {
    ...
    public Deck(int n)
    {
        ...
        playingDeck = ((List)((ArrayList) originalDeck).clone());
    }
    ...
    public List<Card> deal(int numberCards) throws PlayingCardException
    {
        ...
        if(numberCards > remain())
            throw new PlayingCardException("Not enough remaining cards left.");

        while(numberCards > 0) {
            Card dealtCard = playingDeck.remove(playingDeck.size()-1);
            dealCard.add(dealtCard);
            numberCards--;
        }
        return dealCard;
    }
